...
const CRCType CRC16IBM::makeNextChecksum(const CRCType& prevChecksum, unsigned char c) const {
...
const CRCType CRC16IBM::makePrevChecksum(const CRCType& nextChecksum, unsigned char c) const {
...
const CRCType CRC16IBM::computePatch(const CRCType& desiredChecksum, const IFile::OffsetType& desiredPosition, IFile& inputFile, const bool& overwrite) const {
	uint16_t checksum1 = this->computePartialChecksum(inputFile, 0, desiredPosition, this->getInitialXOR());
	uint16_t checksum2 = desiredChecksum ^ this->getFinalXOR();
	...
	size_t i, j;
	for (i = 0, j = 1; i < 2; i ++, j --) {
		patch = this->makePrevChecksum(patch,(checksum1 >>(j << 3)) & 0xff); // differrent order to CRC32
	}
	return patch;
}
